	...
	public function onCommand(CommandSender $sender,Command $cmd,$label, array $args) {
		...
		switch ($cmd->getName()) {
			case "clearinv":
				...
				if ($other) $target->sendMessage(mc::_("ACHTUNG! Dein Inventar wurde von %1% gecleart!", $sender->getName()));
				$sender->sendMessage(mc::_("%1%'s inventory cleared",$target->getName()));
				return true;
			case "clearhotbar":
				InvUtils::clearHotbar($target);
				if ($other) $target->sendMessage(mc::_("Deine Hotbar wurde von %1% gecleart", $sender->getName()));
