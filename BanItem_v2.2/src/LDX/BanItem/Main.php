...
class Main extends PluginBase implements Listener {
  ...
  public function onTouch(PlayerInteractEvent $event) {
    ...
    if($this->isBanned($event->getItem())) {
      if(!($p->hasPermission("banitem") || $p->hasPermission("banitem.bypass"))) {
        $p->sendMessage("§cAchtung!§7 Dieses Item darfst du nicht benutzen.");
        $event->setCancelled();
      }
    }
  }
  ...
  public function onBlockPlace(BlockPlaceEvent $event) {
    ...
    if($this->isBanned($event->getItem())) {
      if(!($p->hasPermission("banitem") || $p->hasPermission("banitem.bypass"))) {
        $p->sendMessage("§cAchtung!§7 Dieses Item darfst du nicht setzen.");
        $event->setCancelled();
      }
    }
  }
  ...
  public function onHurt(EntityDamageEvent $event) {
    if($event instanceof EntityDamageByEntityEvent && $event->getDamager() instanceof Player) {
      ...
      if($this->isBanned($p->getInventory()->getItemInHand())) {
        if(!($p->hasPermission("banitem") || $p->hasPermission("banitem.bypass"))) {
          $p->sendMessage("§cAchtung! §7Dieses Item darfst du nicht benutzen.");
          $event->setCancelled();
        }
      }
    }
  ...
  public function onEat(PlayerItemConsumeEvent $event) {
    ...
    if($this->isBanned($event->getItem())) {
      if(!($p->hasPermission("banitem") || $p->hasPermission("banitem.bypass"))) {
        $p->sendMessage("§cAchtung! §7Dieses Item darfst du nicht essen.");
        $event->setCancelled();
      }
    }
  }
  ...
  public function onCommand(CommandSender $p,Command $cmd,$label,array $args) {
    ...
    if(strtolower($args[0]) == "ban" || strtolower($args[0]) == "unban") {
      ...
      if(!is_numeric($item[0]) || (isset($item[1]) && !is_numeric($item[1]))) {
        $p->sendMessage("§cBitte nutze die Item ID und wenn du es brauchst die DAMAGE.");
        return true;
      }
    }
    if(strtolower($args[0]) == "ban") {
      ...
      if(in_array($i,$this->items)) {
        $p->sendMessage("§cDieses Item ist bereits gebannt.");
      } else {
        array_push($this->items,$i);
        $this->saveItems();
        $p->sendMessage("§aDas Item " . str_replace("#",":",$i) . " wurde gebannt.");
      }
      ...
      if(!in_array($i,$this->items)) {
        $p->sendMessage("§cDieses Item war nicht gebannt.");
      } else {
        array_splice($this->items,array_search($i,$this->items),1);
        $this->saveItems();
        $p->sendMessage("§aDas Item " . str_replace("#",":",$i) . " wurde entbannt.");
      }
    } else if(strtolower($args[0]) == "list") {
      $p->sendMessage("§eBanned item" . (count($this->items) == 1 ? "" : "s") . ": §f" . str_replace("#", ":", implode(", ", $this->items)) . (count($this->items) > 0 ? "." : "§7None."));
    } else {
      return false;
    }
    return true;
