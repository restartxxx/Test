...
class FactionMain extends PluginBase implements Listener {
 ...
 /*   public function getAllAllies($faction){
		...
		if($this->cornerIsInPlot($x + $arm, $z + $arm, $x - $arm, $z - $arm)) {
            ...
            if($this->prefs->get("EnableOverClaim")){
                if($power_sender < $power_claimedBy){
                    $sender->sendMessage($this->formatMessage("This area is aleady claimed by $claimedBy with power $power_claimedBy. Your faction has $power_sender power. You can not overclaim this plot."));
                } else {
                    $sender->sendMessage($this->formatMessage("§6- §3This area is aleady claimed by §e$claimedBy §3with power §b$power_claimedBy §3. Your faction has $power_sender power. Type /f overclaim to overclaim this plot if you want."));
                }
                return false;
            } else {
	...
	public function formatMessage($string, $confirm = false) {
		if($confirm) {
			return "[" . TextFormat::GOLD . "CPE Factions" . TextFormat::WHITE . "] " . TextFormat::GREEN . "$string";
		} else {	
			return "[" . TextFormat::GOLD . "CPE Factions" . TextFormat::WHITE . "] " . TextFormat::RED . "$string";
		}
	}
